...
@implementation IcePopsStickViewController
@synthesize mold;

- (id)init
{
    if([[UIScreen mainScreen] bounds].size.height == 568)
        self = [super initWithNibName:@"IcePopsStickViewController-5" bundle:[NSBundle mainBundle]];
    else
        self = [super init];
...
- (void)viewDidLoad
{
    ...
    UIImageView *moldx = [[UIImageView alloc] initWithFrame:CGRectMake(52, 126, 217, 223)];
    
    if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad)
        moldx.frame = CGRectMake(194, 249, 380, 380);
    
    moldx.image = [UIImage imageNamed:[NSString stringWithFormat:@"p%d.png", mold]];
    [moldx setContentMode:UIViewContentModeScaleAspectFit];
    [self.view insertSubview:moldx atIndex:1];
    [moldx release];
}
...
- (IBAction)nextClick:(id)sender
{
    ...
    else
    {
        [(AppDelegate*)[[UIApplication sharedApplication] delegate] playSoundEffect:27];
        [[NSNotificationCenter defaultCenter] removeObserver:self];
        stick = scroll.contentOffset.x/[UIScreen mainScreen].bounds.size.width + 1;
        
        IcePopsFlavorViewController *icePopsFlavor = [[IcePopsFlavorViewController alloc] init];
        icePopsFlavor.mold = mold;
        icePopsFlavor.stick = stick;
