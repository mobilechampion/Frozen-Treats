//
//  IcecreamConeFirstViewController.m
//  Frozen Treats
//
...
@implementation IcecreamConeFirstViewController

- (id)init
{
    if([[UIScreen mainScreen] bounds].size.height == 568)
        self = [super initWithNibName:@"IcecreamConeFirstViewController-5" bundle:[NSBundle mainBundle]];
    else
        self = [super init];
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    if([[[NSUserDefaults standardUserDefaults] objectForKey:@"icecreamconefirst"] isEqualToString:@"yes"])
    {
    }
    else
    {
        ...
        {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Notice:" message:@"While playing Ice Cream Cones Maker you will see ads. If you purchase the unlock Ice Cream Cones pack, ads will be removed." delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil];
            [alert show];
            [alert release];
            
            [[NSUserDefaults standardUserDefaults] setObject:@"yes" forKey:@"icecreamconefirst"];
...
//    accelerometer.delegate = nil;
    ...
    IcecreamConeFlavorViewController *iceConeFlavor = [[IcecreamConeFlavorViewController alloc] init];
    iceConeFlavor.flavor1 = flavor;
    [self.navigationController pushViewController:iceConeFlavor animated:YES];
    [iceConeFlavor release];
}
...
- (IBAction)flavorSelected:(UIButton*)sender
{
    ...
    flavorImgView.image = [self colorImageWithName:@"flavor_mask.png"];
    maskLayer = [CALayer layer];
    maskLayer.frame = CGRectMake(0, flavorImgView.frame.size.height, flavorImgView.frame.size.width, flavorImgView.frame.size.height);
    maskLayer.contents = (id)[[UIImage imageNamed:@"flavor_mask.png"] CGImage];
    flavorImgView.layer.mask = maskLayer;
