...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self != nil) {
		...
		margin = UIEdgeInsetsMake(DEFAULT_MARGIN, 0, DEFAULT_MARGIN, DEFAULT_MARGIN);
		padding = UIEdgeInsetsMake(DEFAULT_MARGIN, 0, DEFAULT_MARGIN, DEFAULT_MARGIN);
...
//        back.autoresizesSubviews = YES;
       

		self.autoresizingMask = UIViewAutoresizingFlexibleWidth|UIViewAutoresizingFlexibleHeight;
		self.autoresizesSubviews = YES;
		
		self.contentContainer = [[[UIImageView alloc] initWithFrame:self.bounds] autorelease];
        ...
        [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(closePressed:) name:@"IAPHelperProductPurchasedNotification" object:nil];
...
- (UIButton*)closeButton
{
	if (!closeButton)
    {
        ...
        {
            [self.closeButton setImage:[UIImage imageNamed:@"close~ipad.png"] forState:UIControlStateNormal];
            [self.closeButton setFrame:CGRectMake(0, 0, 100, 100)];
        }
        else
        {
...
- (CGRect)closeButtonFrame
{
    ...
    if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad)
        return CGRectMake(f.origin.x - floor(closeButton.frame.size.width *0.5) + 690,
                          f.origin.y - floor(closeButton.frame.size.height*0.5) + 30,
                          closeButton.frame.size.width,
                          closeButton.frame.size.height);
    else
        return CGRectMake(f.origin.x - floor(closeButton.frame.size.width *0.5) + 270,
                          f.origin.y - floor(closeButton.frame.size.height*0.5) + 10,
                          closeButton.frame.size.width,
                          closeButton.frame.size.height);
}
